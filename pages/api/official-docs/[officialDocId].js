...
export default async function handler(req, res) {
  ...
  if (session) {
    ...
    if (req.method === 'GET') {
      try {
        const officialDoc = await getDocuments('official-docs', ObjectId(officialDocId));
    ...
    if (req.method === 'PATCH') {
      ...
      try {
        const response = await patchDocument('official-docs', { _id: ObjectId(officialDocId) }, req.body);
    ...
    if (req.method === 'DELETE') {
      ...
      let fileRef;
      try {
        const document = await getDocument('official-docs', ObjectId(officialDocId), { url: 1 }, { _id: -1 });
      ...
      // Delete from mongoDB
      try {
        const officialDoc = await deleteDocument('official-docs', { _id: ObjectId(officialDocId) });
        return res.status(200).json(officialDoc);
      } catch (error) {
        console.error(error);
        return res.status(500).json(error);
